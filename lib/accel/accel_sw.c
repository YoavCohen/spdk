			   ...
			   sw_accel_crypto_op op)
{
	...
	while (remaining_len) {
		...
		op((uint8_t *)key->key2, (uint8_t *)key->key1, (uint8_t *)iv, crypto_len, src,
		   dst);
...
sw_accel_create_aes_xts(struct spdk_accel_crypto_key *key)
{
	...
	if (!key->key1 || !key->key2) {
		SPDK_ERRLOG("key1 or key2 are missing\n");
		return -EINVAL;
	}

	if (!key->key1_size || key->key1_size != key->key2_size) {
		SPDK_ERRLOG("key1 size %zu is not equal to key2 size %zu or is 0\n", key->key1_size,
			    key->key2_size);
		return -EINVAL;
	}
	...
	default:
		SPDK_ERRLOG("Incorrect key size  %zu, should be %d for AEX_XTS_128 or %d for AES_XTS_256\n",
			    key->key1_size, ACCEL_AES_XTS_128_KEY_SIZE, ACCEL_AES_XTS_256_KEY_SIZE);
		free(key_data);
		return -EINVAL;
	}
