		 ...
		 const char *direction)
{
	...
	if (!arg_size) {
		SPDK_ERRLOG("%s arg of zero length attached at %zu:%zu\n", direction, offs->iov_offs,
			    offs->buf_offs);
		return NULL;
	}

	if (size > arg_size) {
		SPDK_ERRLOG("%s arg is too small (%zu > %zu) at %zu:%zu\n", direction, size, arg_size,
...
do_readdir_common(struct fuse_io *fuse_io, bool plus)
{
	...
	err = spdk_fsdev_op_readdir(fuse_io_desc(fuse_io), fuse_io->ch, fuse_io->hdr.unique,
				    fuse_io->hdr.nodeid, fsdev_io_d2h_u64(fuse_io, arg->fh),
				    fsdev_io_d2h_u32(fuse_io, arg->offset),
