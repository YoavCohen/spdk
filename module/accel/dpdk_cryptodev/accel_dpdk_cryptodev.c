...
/* Max length in byte of a crypto opertaion */
...
accel_dpdk_cryptodev_poller(void *args)
{
	...
	while (!TAILQ_EMPTY(&crypto_ch->queued_cry_ops)) {
		...
		enqueued = rte_cryptodev_enqueue_burst(op_to_resubmit->qp->device->cdev_id,
						       op_to_resubmit->qp->qp,
 ...
 * it to the @orig_mbuf. */
static inline int
accel_dpdk_crytodev_mbuf_chain_remainder(struct accel_dpdk_cryptodev_task *task,
		struct rte_mbuf *orig_mbuf,
...
accel_dpdk_find_key_handle_in_channel(struct accel_dpdk_cryptodev_io_channel *crypto_ch,
				      struct accel_dpdk_cryptodev_key_priv *key, struct accel_dpdk_cryptodev_key_handle **_key_handle)
{
	...
	if (key->driver == ACCEL_DPDK_CRYPTODEV_DRIVER_MLX5_PCI) {
		...
		*_key_handle = TAILQ_FIRST(&key->dev_keys);
	}
		...
		struct accel_dpdk_cryptodev_task *task)
{
	...
	uint32_t buf_len = spdk_min(task->base.block_size, sgl->iov->iov_len - sgl->iov_offset);
	...
	remainder = buf_len - phys_len;
	while (remainder) {
		buf_len = spdk_min(remainder, sgl->iov->iov_len - sgl->iov_offset);
		buf_addr = sgl->iov->iov_base + sgl->iov_offset;
		rc = accel_dpdk_crytodev_mbuf_chain_remainder(task, mbuf, buf_addr, buf_len);
		if (spdk_unlikely(rc)) {
			return rc;
		}
		spdk_iov_sgl_advance(sgl, buf_len);
				  ...
				  struct accel_dpdk_cryptodev_task *task)
{
	...
	rc = accel_dpdk_find_key_handle_in_channel(crypto_ch, priv, &key_handle);
	if (rc) {
	...
	if (task->base.s.iovcnt == task->base.d.iovcnt) {
		for (iov_index = 0; iov_index < task->base.s.iovcnt; iov_index++) {
			if (task->base.s.iovs[iov_index].iov_base != task->base.d.iovs[iov_index].iov_base ||
			    task->base.s.iovs[iov_index].iov_len != task->base.d.iovs[iov_index].iov_len) {
				inplace = false;
				break;
			}
		}
...
accel_dpdk_cryptodev_get_next_device_qpair(enum accel_dpdk_cryptodev_driver_type type)
{
	...
	TAILQ_FOREACH_SAFE(device, &g_crypto_devices, link, device_tmp) {
		...
		TAILQ_FOREACH(qpair, &device->qpairs, link) {
			if (!qpair->in_use) {
				qpair->in_use = true;
				/* Move the current device to the end of the list in order to use other devices of the same type in round-robin way */
...
accel_dpdk_cryptodev_submit_tasks(struct spdk_io_channel *_ch, struct spdk_accel_task *_task)
{
	...
	assert(_task->op_code == ACCEL_OPC_ENCRYPT || _task->op_code == ACCEL_OPC_DECRYPT);
		...
		struct accel_dpdk_cryptodev_key_handle *key_handle)
{
	...
	case ACCEL_DPDK_CRYPTODEV_CIPHER_AES_CBC:
		key_handle->cipher_xform.cipher.key.data = key->key1;
		key_handle->cipher_xform.cipher.key.length = key->key1_size;
	...
	case ACCEL_DPDK_CRYPTODEV_CIPHER_AES_XTS:
		...
		key_handle->cipher_xform.cipher.key.length = key->key1_size + key->key2_size;
		...
		enum accel_dpdk_crypto_dev_cipher_type cipher, struct spdk_accel_crypto_key *key)
{
	...
	switch (cipher) {
	case ACCEL_DPDK_CRYPTODEV_CIPHER_AES_CBC:
		if (!key->key1 || !key->key1_size) {
			SPDK_ERRLOG("ACCEL_DPDK_CRYPTODEV_AES_CBC requires key1\n");
			return -1;
		}
		...
		break;
	case ACCEL_DPDK_CRYPTODEV_CIPHER_AES_XTS:
		if (!key->key1 || !key->key1_size || !key->key2 || !key->key2_size) {
			SPDK_ERRLOG("ACCEL_DPDK_CRYPTODEV_AES_XTS requires key1 and key2\n");
			return -1;
		}
		break;
	default:
	...
	case ACCEL_DPDK_CRYPTODEV_CIPHER_AES_CBC:
		...
		if (key->key1_size != ACCEL_DPDK_CRYPTODEV_AES_CBC_KEY_LENGTH) {
			SPDK_ERRLOG("Invalid key1 size %zu for cipher %s, should be %d\n", key->key1_size,
	...
	case ACCEL_DPDK_CRYPTODEV_CIPHER_AES_XTS:
		...
		case ACCEL_DPDK_CRYPTODEV_DRIVER_MLX5_PCI:
			if (key->key1_size != ACCEL_DPDK_CRYPTODEV_AES_XTS_256_BLOCK_KEY_LENGTH &&
			    key->key1_size != ACCEL_DPDK_CRYPTODEV_AES_XTS_512_BLOCK_KEY_LENGTH) {
				SPDK_ERRLOG("Invalid key1 size %zu for driver %s, cipher %s, supported %d or %d\n",
					    key->key1_size, g_driver_names[ACCEL_DPDK_CRYPTODEV_DRIVER_MLX5_PCI],
					    g_cipher_names[ACCEL_DPDK_CRYPTODEV_CIPHER_AES_XTS],
		...
		case ACCEL_DPDK_CRYPTODEV_DRIVER_AESNI_MB:
			if (key->key1_size != ACCEL_DPDK_CRYPTODEV_AES_XTS_128_BLOCK_KEY_LENGTH) {
				SPDK_ERRLOG("Invalid key1 size %zu supported %d\n", key->key1_size,
...
accel_dpdk_cryptodev_key_deinit(struct spdk_accel_crypto_key *key)
{
	...
	if (priv->xts_key) {
		memset(priv->xts_key, 0, key->key1_size + key->key2_size + 1);
	}
	free(priv->xts_key);
	free(priv);
}
...
accel_dpdk_cryptodev_key_init(struct spdk_accel_crypto_key *key)
{
	...
	if (!key->param.driver_name || !key->param.cipher) {
		SPDK_ERRLOG("Driver name or cipher are missing\n");
		return -EINVAL;
	}

	if (strcmp(key->param.driver_name, ACCEL_DPDK_CRYPTODEV_QAT) == 0) {
		driver = ACCEL_DPDK_CRYPTODEV_DRIVER_QAT;
	} else if (strcmp(key->param.driver_name, ACCEL_DPDK_CRYPTODEV_AESNI_MB) == 0) {
		driver = ACCEL_DPDK_CRYPTODEV_DRIVER_AESNI_MB;
	} else if (strcmp(key->param.driver_name, ACCEL_DPDK_CRYPTODEV_MLX5) == 0) {
		driver = ACCEL_DPDK_CRYPTODEV_DRIVER_MLX5_PCI;
	} else {
		SPDK_ERRLOG("Unsupported driver %s\n", key->param.driver_name);
		return -EINVAL;
	}
	...
	if (cipher == ACCEL_DPDK_CRYPTODEV_CIPHER_AES_XTS) {
		...
		priv->xts_key = calloc(key->key1_size + key->key2_size + 1, sizeof(char));
		if (!priv->xts_key) {
			SPDK_ERRLOG("Memory allocation failed\n");
			return accel_dpdk_cryptodev_key_deinit(key), -ENOMEM;
		}
		memcpy(priv->xts_key, key->key1, key->key1_size);
		memcpy(priv->xts_key + key->key1_size, key->key2, key->key2_size);
	}
...
accel_dpdk_cryptodev_write_config_json(struct spdk_json_write_ctx *w)
{
	...
	spdk_json_write_named_string(w, "method", "dpdk_cryptodev_accel_enable");
